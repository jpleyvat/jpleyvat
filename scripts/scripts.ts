...
for (let i: number = 0; i < navigation.length; i++) {
  navigation[i].children[0].addEventListener("click", (event) => {
    ...
    let interval: number =
      (Math.abs(initialPosition - (pageNumber - 1) * 100) * operation) / 100;
  });
}
...
function scrollUp(): void {
  ...
  if (container.style.bottom != "" && container.style.bottom != "0%") {
    ...
    let animation: number = setInterval((): void => {
      if (
        getpPropertyValue(container.style.bottom) === initialPosition - 100 ||
...
let alreadyAnimated = [false, false, false, false, false, false];

function animatePage(page: number) {
  switch (page) {
    case 2:
...
function animateAbout(): void {
  let about: any = document.getElementsByClassName("about")[0];
  Object.keys(about.children).forEach((key) => {
    about.children[key].style.animationPlayState = "running";
  });
}

function animateProjects(pageid: string): void {
